...
class TestPutBusiness(BaseTestCase):
    ...
    def test_business_can_be_edited(self):
        ...
        res = self.make_request('/api/v1/businesses/5', data=self.update_data, method='put')
        result = json.loads(res.data.decode())
    ...
    def test_forbidden_business(self):
        ...
        with self.app.app_context():
            ...
            access_token = create_access_token(identity=2, expires_delta=datetime.timedelta(minutes=5))
            self.header['Authorization'] = 'Bearer ' + access_token
            res = self.make_request('/api/v1/businesses/6', data=self.business_data, method='put')
            result = json.loads(res.data.decode())
...
class TestDeleteBusiness(BaseTestCase):
    ...
    def test_business_can_be_deleted(self):
        ...
        self.register_business(self.business_data)
        res = self.make_request('/api/v1/businesses/1', data={'password':'test1234'}, method='delete')
        result = json.loads(res.data.decode())
        self.assertEqual(result['message'], 'Business 1 deleted')

    def test_null_password(self):
        """Test delete with null password"""
        self.get_login_token()
        res = self.make_request('/api/v1/businesses/1', data={}, method='delete')
        result = json.loads(res.data.decode())
    ...
    def test_not_exist_business(self):
        ...
        self.get_login_token()
        res = self.make_request('/api/v1/businesses/10', data={'password':'test1234'}, method='delete')
        result = json.loads(res.data.decode())
        self.assertEqual(result['message'], 'The business 10 is not available')
